...
import { fcfs } from './fcfs.ts';
import type { Process } from '../types/index.ts';

test('fcfs schedules processes in arrival order with pid tie-breaks', () => {
  ...
  assert.equal(result.algorithm, 'fcfs');
  assert.deepEqual(result.ganttBlocks, [
    { processId: 1, start: 0, end: 8 },
    { processId: 2, start: 8, end: 12 },
    { processId: 3, start: 12, end: 21 },
    { processId: 4, start: 21, end: 26 },
  ]);
  assert.deepEqual(result.processMetrics, {
    ...
    3: { completionTime: 21, turnaroundTime: 19, waitingTime: 10 },
    4: { completionTime: 26, turnaroundTime: 23, waitingTime: 18 },
  });
  assert.deepEqual(result.averages, {
    completionTime: 16.75,
    turnaroundTime: 15.25,
    waitingTime: 8.75,
  });
});

test('fcfs inserts an idle block and handles empty input', () => {
  assert.deepEqual(fcfs([]), {
    algorithm: 'fcfs',
    ganttBlocks: [],
    processMetrics: {},
